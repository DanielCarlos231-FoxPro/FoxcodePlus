...
define class FoxCodePlusMain as custom
	...
	*dimension NoIntelliSense[1,3] 					&&& comandos e funcoes que não devem ter IntelliSense do foxcodeplus
	...
	dimension Environment[10]						&&& array que controla o ambiente da IDE do VFP para o Foxcodeplus
...
*!*			into array this.NoIntelliSense
	...
	*/-------------------------------------------------------------------------------------------------/*
	protected procedure ChkReportDesigner()
		...
		*	WAIT WINDOW "Report is on top..."+TTOC(DATETIME()) NOWAIT NOCLEAR
			local lnReportHwnd
			*- capturo o hwnd da tela do debug
			lnReportHwnd = this.getWindowHwnd("Report Designer")

			*- para caso queira adicionar a mesma verificação que o Debugger
			*- usar essa variável
			This.HasReportOnTop = .t.
			*unbindevents(0, WM_KEYUP)
			
			bindevent( lnReportHwnd, WM_DESTROY, this, "ReStartIntelliSense", 4 )
			return .f.
		ENDIF
	endproc
	
	protected procedure ReportDestroy(hwnd as integer, msg as integer, wparam as integer, lparam as integer)
		WAIT WINDOW "Fechando Report as "+TTOC(DATETIME()) NOWAIT NOCLEAR 
		IF TYPE("this.IntelliSense") = "O"
			WAIT WINDOW "FoxCodePlus foi finalizado as "+TTOC(DATETIME()) NOWAIT NOCLEAR 
			this.IntelliSense.Showed = .F.
			this.IntelliSense.hide()
			sys(2030,1)
		ENDIF
	...
	*/-----------------------------------------------------------------------------------------------------
	*/*/ (DCA) - 24/06/2023 - nº (#000008) - Pega o bloco inteiro de linhas dividas pelo ";"
	*/-----------------------------------------------------------------------------------------------------
	protect procedure GetTextBlockLine
			lparameters plnLineNo
		...
		*- caso exista procuro a linha onde foi iniciado o comando.
		do while .t.
			...
			if lnChkLineNo > 0
				...
				lcString = chrtran(lcString,chr(9)," ")
		...
		*/ faço uma concatenação deste a linha inicial 
		*/ até a linha final do bloco de texto feito com o ";"
		lcString = ""
		do while .t.
	...
	*/------------------------------------------------------------------------------------------------	
	procedure GetDatabase
		...
		lcDatabases = "tmp"+sys(2015)
		
		
		*/-----------------------------------------/*
		Try
			if asqlhandles(laCnx) > 0
				...
				DO WHILE lnIdHandle>0 AND lnIdHandle <= Alen(laCnx,1)
					...
					If sqltables(laCnx[lnIdHandle],"TABLE",lcDatabases) = 1 
						...
						Select (lcDatabases)
						Select Table_cat, Table_schem From (lcDatabases) Group by Table_cat, Table_schem into cursor (lcDatabases) 
...
*!*					endscan
	...
	*/------------------------------------------------------------------------
	*/ NEW (DCA) - 26/02/2023 - nº (#000006) STATUS (EM TESTES)
	*/ 				I get all variables At Run-Time - In Development  
	*/------------------------------------------------------------------------
	protected procedure GetConstantsRunTime
		lparameters plcWord, plcIncObj
		  
		LOCAL lnItemsFound, lolCTmpFil, lolCTmpStr, inLines, inImage, lnCounter
		LOCAL loccnomvar, loccvisvar, locctipvar, loccvalvar, loccToolTp, lcSafety
		
		LOCAL array laVaribles(1,3) 
		lnItemsFound = 0
		inLines		 = 0 
		inImage		 = 11
		
		&& LIST VARIABLES FORM MEMORY AT RUN-TIME
		lolCTmpFil = This.TmpFile
		
		lcSafety = set("Safety")
		set safety off 
		list memory to file (lolCTmpFil) noconsole
		set safety &lcSafety

		lolCTmpStr = filetostr(lolCTmpFil) 
		alines(lolATmpFil, lolCTmpStr)
		
		&& loop through the array of lines to get váriables
		FOR lnCounter = 1 TO alen(lolATmpFil)
		
			&& save the name, type and visibility of variables
			loccnomvar = Alltrim(GetWordnum(lolATmpFil[lnCounter],1))
			loccvisvar = Alltrim(GetWordnum(lolATmpFil[lnCounter],2))
			locctipvar = Alltrim(GetWordnum(lolATmpFil[lnCounter],3))
			inImage	  = 11
			
	   	&& if the line has "variables defined," than it is the end of the declared Variables
	   	if "variables defined," $ lolATmpFil[lnCounter]
				exit
			endif
		
			&& Skip the loop 
			if empty(lolATmpFil[lnCounter]) 				  			OR; && if Current line is empty
				(inlist(lolATmpFil[lnCounter],"(",")"))  			or; && if contains "(" or ")"  
				empty(substr(lolATmpFil[lnCounter],1,1)) 			OR; && if first caracter is empty
				(locctipvar $ "O" AND !plcIncObj)					or; && if is an Object and the object should not be shown
				!InList(Lower(loccvisvar),"pub") && ,"priv","local" 
															&& Não vai ser mostrado "priv","local"
															&& As variáveis como local estavam mostrando também as do FoxCodePlus, o que não era o desejado
															&& Por consitência decidi remover também as Privates e mostrar somente as Public
				loop
			endif
			
			&& if variable name is not empty than add to the Array of Variables
			IF !Empty(Alltrim(GetWordnum(lolATmpFil[lnCounter],1))) 
				
				&& Save the Text of the ToolTip
				loccToolTp = "This is a Constant instantiated in memory (at run-time)"
				try
					DO CASE 
						CASE locctipvar = "C" && C = Strings				
							loccvalvar = &loccnomvar
							
							TEXT TO loccToolTp NOSHOW TEXTMERGE PRETEXT 2
								<<loccnomvar>> (<<locctipvar>>) = "<<loccvalvar>>"
								This is a Constant instantiated in memory (at run-time)
							ENDTEXT
						 
						CASE locctipvar $ "N,L,D,T" && N = numbers, L = boolean, D = Date, T = DateTime
							loccvalvar = &loccnomvar
						...
						CASE locctipvar $ "O" AND plcIncObj && Object
							
							LOCAL loccclsnam, loccbascls, loccclslib, loccvalvar  
							
							IF !ISNULL(&loccnomvar)
								loccvalvar = "(Object)"
								loccclsnam = &loccnomvar..class
								loccbascls = &loccnomvar..baseclass
								loccclslib = iif(empty(&loccnomvar..classlibrary), "(None)", &loccnomvar..ClassLibrary)
								inImage	  = 20
							ELSE
								loccvalvar = ".NULL."
								loccclsnam = "(None)"
							...
							TEXT TO loccToolTp NOSHOW TEXTMERGE PRETEXT 2
								Object <<loccnomvar>> (<<locctipvar>>) =  <<loccvalvar>>
								Class: <<loccclsnam>>
								BaseClass: <<loccbascls>> 
								ClassLibrary: <<loccclslib>>
						...
						CASE locctipvar = "M" && Memory
							TEXT TO loccToolTp NOSHOW TEXTMERGE PRETEXT 2
								<<loccnomvar>> (<<locctipvar>>) = [Memor]
								This is a Constant instantiated in memory (at run-time)
							ENDTEXT
				...
				inLines = inLines + 1
				dimension laVaribles[inLines,3]
				
				laVaribles[inLines,1] = loccnomvar 
				laVaribles[inLines,2] = inImage
				laVaribles[inLines,3] = loccToolTp
												
			endif
		
		ENDFOR
		
		&& If no variable was found than return
		IF inLines = 0
			RETURN lnItemsFound
		endif
		
		&& loop through the array of vaiables and add them tho the IntelliSense
		FOR lnCounter = 1 TO alen(laVaribles,1)
			
			if empty(laVaribles[lnCounter,1]) or not this.ChkIncremental(plcWord,laVaribles[lnCounter,1])
				loop
			endif 
			...
			do case 
				...
				*- lista de variaveis "m."
				case lower(lcLastWord) == "m." and occurs(".",lcLastWord)=1
					...
					&& (DCA) - ENHANCED  - 26/02/2023 - nº (#000006) STATUS (EM TESTES)
					lnLines = lnLines + this.GetConstantsRunTime(lcLastWord,.t.) && pegar constantes (at Run-Time)
					&& (DCA) - ENHANCED  - 26/02/2023 - nº (#000006) STATUS (EM TESTES)
					this.IncrementalResult = .t.
	...
	*** <remarks></remarks>
	protected procedure IsInParentheses
		lparameters plcText 
		local i, lcChar, lInParenthesis, lcParenthesisChar
		...
		for i = 1 to len(plcText)
			...
			if !lInParenthesis
				if lcChar = "("
					lcParenthesisChar = lcChar
				endif
			else
				if (lcParenthesisChar="(" and lcChar=")") 
					lInParenthesis = .t.
				endif
			endif
		ENDFOR
		
		return lInParenthesis
	endfunc
	...
	*/------------------------------------------------------------------------------------------------	
	procedure SetFoxcodePlusEnvironment
		...
		if plnMode =  1
			...
			this.Environment[10] = Set('TABLEPROMPT') && (DCA) - 26/06/2023 - Desativa o Prompt de tabela que abre 
													  &&  automaticamente do FoxPro, quando o Cursor/Tabela não existe
			...
			*sys(2030,0)
			...
			&& (DCA) - 26/06/2023 - Desativa o Prompt de tabela que abre 
			&&  automaticamente do FoxPro, quando o Cursor/Tabela não existe
			if this.Environment[10]="ON"
				set tableprompt on
			endif
